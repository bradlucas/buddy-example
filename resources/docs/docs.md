...
The `home-routes` handler in the `buddy-example.routes.home` namespace
...
The `home-page` function will in turn call the `buddy-example.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `buddy-example.handler` namespace:
...
Request middleware functions are located under the `buddy-example.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `buddy-example.dev-middleware` namespace found in
